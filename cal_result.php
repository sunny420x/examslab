...
<html>
    <head>
        <title>Exams Lab | Sunny420x</title>
        <?php
        ...
        <div class="container my-10 p-20">
            <div class="w-100 card white rounded p-30">
                <div>
                    ...
                    ?>
                    <h2 class="bl-red">เปอร์เซ็นต์การตอบถูกของข้อต่างๆ</h2>
                    <p>มีนักศึกษาทำแบบทดสอบทั้งหมดจำนวน <?=$counts_all_peoples?> คน</p>
                    <p>ทำคะแนนได้เฉลี่ยอยู่ที่ <?=round($sum/$counts_all_peoples, 2)?> คะแนน</p>
                    <table class="full">
                            <?php
                            $i = 1;
                            $get_amounts = mysqli_prepare($db, "SELECT id FROM questions WHERE name = ? ORDER BY id ASC");
                            ...
                            ?>
                                <td><?=$info['counts']?> คน (<?=round($info['counts'] / $counts_all_peoples * 100, 2)?>%)</td>
                            <?php
                                }
                            ?>
                            <?php
